...
describe('ReaderCardVisual', () => {
  ...
  ])('renders one bounded authorized %s visual and opens it in a media dialog', async (_kind, item, label, href) => {
    const { container } = render(<ReaderCardVisual item={item} />)

    const trigger = screen.getByRole('button', {
      name: _kind === 'video' ? /Play video:/ : /View larger image:/,
    })
    expect(trigger.className).toContain('max-h-40')
    expect(trigger.textContent).toContain(_kind === 'video' ? 'Play video' : label)
